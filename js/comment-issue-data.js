...
function dealWtihContentStr(contentStr) {
    if (contentStr == undefined || contentStr == "") {
        contentStr = "empty content!";
    }

    // 替换图片
    contentStr = contentStr.replace(/![\s\w\](?:http(s)?:\/\/)+[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+\)/g, "[photo]");

    // 替换网址
    contentStr = contentStr.replace(/(?:http(s)?:\/\/)+[\w.-]+(?:\.[\w\.-]+)+[\w\-\._~:/?#[\]@!\$&'\*\+,;=.]+/g, "[site]");
    ...
    if (COMMENT_ARR.length > 0) {
        ...
        for (var i = 0; i < COMMENT_ARR.length; i++) {
            ...
            htmlContentWidget +=
                ...
                "<div class='item-header-text'><a href='" + item.userUrl + "' target='_blank'>" + item.userName + "</a>&nbsp;published" + getDateDiff(new Date(item.date).getTime()) + "</div>" + "<div class=\"item-text\"><a href =\"" + item.url + '#comment-container\">' + contentStr + "</a></div>" +
                "</div>";
        }
        ...
        $(".body_hot_comment").html("No recorded data！");
    }
}
...
function loadIndexHotData(authorizationToken) {
    ...
    if ($("#index_hot_div").length > 0) {
        ...
        ajaxReqForGitHub(repoIssuesUrl + "?per_page=10&sort=comments", authorizationToken, function (result) {
            ...
            hotDiv.html("");
            if (hotContent == "") {
                hotDiv.append("No recorded data！");
            } else {
            ...
            if (typeof Valine == "function") {
                ...
                if (valine == undefined || valine == null) {
                    valine = new Valine({
                        ...
                        appId: appId,
                        appKey: appKey,
                        placeholder: 'would you like to give me some advice！',
                        avatar: 'mp',
                        avatarForce: false,
                        meta: ["nick", "mail", "link"],
                        pageSize: 10,
